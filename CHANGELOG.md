...
- `qmd vsearch` and `qmd query` now run through a running `qmd mcp --http`
  serving the same index instead of loading the models in-process. The server
  writes `~/.cache/qmd/mcp.port` (scoped per `--index` like `mcp.pid`) and
  answers `POST /v1/search`; `GET /health` reports `dbPath` and `pid`. The CLI
  routes only on a matching `dbPath`, output is identical, and any failure
  falls through to the in-process path. `--no-daemon` / `QMD_NO_DAEMON=1` opt
  out, `QMD_DEBUG=1` logs the decision, `qmd status` shows the daemon URL. BM25
  `qmd search` is not routed.
